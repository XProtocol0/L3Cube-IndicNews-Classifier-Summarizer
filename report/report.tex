% ICVGIP-style technical report (uses the CVPR LaTeX class)
% Compile with: pdflatex report.tex
\documentclass[10pt,twocolumn,letterpaper]{article}

\usepackage[final]{cvpr}      % camera-ready style; for review use [review]
\usepackage{times}

% Additional packages not already pulled in by cvpr.sty
\usepackage{epsfig}
\usepackage{array}
\usepackage[pagebackref,breaklinks,colorlinks,citecolor=blue!60!black,linkcolor=blue!60!black,urlcolor=blue!60!black]{hyperref}

% CVPR papers do not get a number
\def\cvprPaperID{****}
\def\confName{ICVGIP}
\def\confYear{2026}
...
%%%%%%%%% TITLE
\title{Personalised Newsletter Generator: \\ Zero-Shot Indian News Classification and LLM-Based Summarisation}

\author{KSPSVLN Siddardha Kumar Kavuri\\(2025201061)
\and Piyush Kumar Agarwal\\(2025201082)
\and Sankalp Chaturvedi\\(2025201083)
\and Aviral Tyagi\\(2025201086)}
...
\medskip
\noindent\textbf{Live demo.} \href{https://huggingface.co/spaces/piyukr26/SMAI_A3}{Hugging Face Space} \\
\textbf{Source code.} \href{https://github.com/XProtocol0/L3Cube-IndicNews-Classifier-Summarizer}{GitHub Repository} \\
\textbf{Demo video.} \href{https://drive.google.com/drive/folders/15FgeyxjyEIJubzcbA3jKG0MLjuL8n4sV?usp=sharing}{Google Drive Video}
...
Figure~\ref{fig:arch} summarises the pipeline. The app is structured as a single-process Streamlit application with caching at three layers (\texttt{cache\_resource} for the heavy classifier, \texttt{cache\_data} for both feed fetches and summaries) so that repeated user interactions remain responsive.
